...
$icsDateTime = '';
if ($entryTime !== null) {
    $dtStart = new DateTime($ticket['event_date'] . ' ' . $entryTime . ':00', new DateTimeZone('Europe/Berlin'));
    $dtEnd = (clone $dtStart)->modify('+4 hours');
    $icsDateTime = "DTSTART:" . $dtStart->setTimezone(new DateTimeZone('UTC'))->format('Ymd\\THis\\Z') . "\r\n"
        . "DTEND:" . $dtEnd->setTimezone(new DateTimeZone('UTC'))->format('Ymd\\THis\\Z') . "\r\n";
} else {
    $icsDateTime = "DTSTART;VALUE=DATE:" . str_replace('-', '', $ticket['event_date']) . "\r\n";
}
